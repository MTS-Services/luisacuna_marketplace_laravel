...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        if ($datas instanceof LengthAwarePaginator) {
            $datas = $datas->getCollection()->load('creater_admin');
        } else {
            // Handle the case when the getPaginatedData method does not return a LengthAwarePaginator object
            // For example, you can throw an exception or log an error message
            Log::error('getPaginatedData method does not return a LengthAwarePaginator object');
        }
        ...
        $actions = [
            [
                'key' => 'id',
                'label' => 'View',
                'route' => 'admin.gm.tag.show',
                'encrypt' => true
            ],
            [
                'key' => 'id',
                'label' => 'Edit',
                'route' => 'admin.gm.tag.edit',
                'encrypt' => true
            ],
            [
                'key' => 'id',
        ...
        return view('livewire.backend.admin.game-management.tag.index', [
    ...
    {
        try {
            $dataStatus = TagStatus::from($status);

            match ($dataStatus) {
                TagStatus::ACTIVE => $this->service->updateStatusData($id, TagStatus::ACTIVE),
                TagStatus::INACTIVE => $this->service->updateStatusData($id, TagStatus::INACTIVE),
                default => null,
            };
    ...
    public function executeBulkAction(): void
    {
        ...
        try {
            match ($this->bulkAction) {
                ...
                'active' => $this->bulkUpdateStatus(TagStatus::ACTIVE),
                'inactive' => $this->bulkUpdateStatus(TagStatus::INACTIVE),
                default => null,
            };
    ...
    protected function bulkUpdateStatus(TagStatus $status): void
    {
