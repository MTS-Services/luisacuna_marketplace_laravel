<div>
    ...
    <a
        href="{{ route('game.buy', ['gameSlug' => $gameSlug, 'categorySlug' => $categorySlug, 'productId' => encrypt($data->id)]) }}">
        <!-- Card -->
        <div class="bg-bg-primary dark:bg-bg-secondary rounded-2xl p-4 shadow-lg transition">
                ...
                <div class="flex items-center space-x-1 text-text-secondary text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 stroke-text-primary" viewBox="0 0 15 15"
                        ...
                        <path
                            ...
                            stroke="currentColor" stroke-width="1.36236" stroke-linecap="round"
            ...
            <a href="{{ route('profile', ['username' => $data?->user?->username ?? '']) }}"
                ...
                @php
                    $positiveFeedbacks = $data?->user
                        ?->feedbacksReceived()
                        ->where('type', App\Enums\FeedbackType::POSITIVE->value)
                        ->count();
                    $negativeFeedbacks = $data?->user
                        ?->feedbacksReceived()
                ...
                <div class="w-full ">
                    ...
                    <div class="flex items-center space-x-2 mt-1">
                        ...
                        <p class="text-text-secondary text-xs">
                            {{ feedback_calculate($positiveFeedbacks, $negativeFeedbacks) }}%<span
