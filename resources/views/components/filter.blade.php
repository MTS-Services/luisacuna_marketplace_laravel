    ...
    <div class="flex items-center justify-between gap-4 mt-3.5">
        ...
        <div class="min-w-30 flex items-center justify-between gap-2 relative" x-data={filter:false}>
            ...
            <button @click="filter = !filter"
                class="flex items-center gap-2 px-4 py-2.5 bg-bg-primary rounded border! border-zinc-700!">
                ...
                <div class="bg-bg-primary rounded-md p-4">
                    <div class="flex flex-col gap-2">
                        <button wire:click="sortBy('asc')" @click="filter = false"
                            class="text-left px-3 py-2 rounded transition {{ $sortOrder === 'asc' ? 'bg-zinc-700' : 'hover:bg-zinc-400' }}">
                            {{ __('A-Z') }}
                        </button>
                        <button wire:click="sortBy('desc')" @click="filter = false"
                            class="text-left px-3 py-2 rounded transition {{ $sortOrder === 'desc' ? 'bg-zinc-700' : 'hover:bg-zinc-400' }}">
                            {{ __('Z-A') }}
                        </button>
                        <button wire:click="sortBy('default')" @click="filter = false"
                            class="text-left px-3 py-2 rounded transition {{ $sortOrder === 'default' ? 'bg-zinc-700' : 'hover:bg-zinc-400' }}">
                            {{ __('Default') }}
                        </button>
                    </div>
                </div>
