        ...
        <div class="flex flex-col h-full">
            ...
            <nav class="flex-1 px-3 py-4 space-y-2">
                ...
                <a href="{{ route('user.notification') }}" wire:navigate
                    @click="$root.sidebarOpen = false"
                    class="flex items-center space-x-2 sm:space-x-3 px-2 sm:px-3 py-2 rounded-lg transition-all text-text-white hover:bg-pink-500/50 {{ $pageSlug === 'notification' ? 'bg-pink-500' : 'bg-pink-300 dark:bg-zinc-950' }}">
                    <flux:icon name="bell" class="w-4 h-4 sm:w-5 sm:h-5 text-text-white" />
                    <span class="text-xs sm:text-sm lg:text-base font-medium text-text-white">{{ __('Notification') }}</span>
                </a>
                <!-- Feedback Link -->
