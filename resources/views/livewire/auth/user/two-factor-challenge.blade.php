<div class="max-w-md mx-auto bg-bg-primary dark:bg-bg-secondary rounded-2xl shadow-glass-card p-8 my-10 glass-card">
    ...
    <div class="mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-800">
        ...
        <!-- Recovery Code Form -->
        <form method="POST" action="{{ route('two-factor.login.store') }}" id="recoveryForm" class="hidden mt-4 space-y-4">
            @csrf
            <div>
    ...
    <div class="mt-6 text-center">
        <a href="{{ route('login') }}" class="text-sm text-accent hover:text-accent-content dark:text-accent-foreground transition-colors">
            ← Back to Login
        </a>
    </div>
