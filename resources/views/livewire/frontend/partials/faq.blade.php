        ...
        <div class="container py-8 flex justify-center wrap flex-col" x-data="{ active: 0, tab: 'buyers' }">
            ...
            <div class="max-w-xs mx-auto flex justify-between mb-8 bg-bg-info rounded-full px-3 py-3">
                <button @click="tab = 'buyers'; active = 0" 
                    :class="tab === 'buyers' ? 'bg-bg-secondary px-5 py-3 rounded-full shadow-lg text-white' :
                        'text-text-secondery px-5 py-3'"
                    class="transition-colors duration-300 font-medium">
                    {{ 'For Buyers' }}
                </button>
                <button @click="tab = 'sellers'; active = 0" 
                    :class="tab === 'sellers' ? 'bg-bg-secondary px-5 py-3 rounded-full shadow-lg text-white' :
                        'text-text-secondery px-5 py-3'"
                    class="transition-colors duration-300 font-medium">
                    {{ __('For Sellers') }}
                </button>
