<header x-data="{ mobileMenuOpen: false, notification: false, dropdown: '', globalSearchModal: false, open: '' }"
    ...
    <div class="2xl:container-wide container  px-4 py-4 flex items-center justify-between relative" x-cloak>
        ...
        <div class="flex items-center">
            <button @click="globalSearchModal = true"
                ...
                <flux:icon name="magnifying-glass" class="w-6 h-6 text-text-white" />
            </button>
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                class="md:hidden ml-2 inline-flex items-center justify-center p-2 rounded-md text-text-secondary hover:text-text-white hover:bg-bg-secondary focus:outline-none focus:ring-2 focus:ring-inset focus:ring-text-text-white">
    ...
    <div x-show="notification" x-cloak @click.outside="notification = false"
        x-transition:enter="transition ease-out duration-100"
        class="absolute top-0 right-0 w-[90%] xs:w-3/4 md:max-w-[380px] bg-bg-primary backdrop:blure-md z-100  transition-all duration-300 h-screen text-text-white shadow-lg overflow-y-auto">
        <div class="mb-2">
            <!-- Header -->
