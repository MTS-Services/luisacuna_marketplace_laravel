<div class="flex flex-col h-full">
    @if ($conversation)
        {{-- Chat Header --}}
        <div class="p-3 sm:p-4 flex gap-10 items-center bg-bg-info rounded-t-lg">
            <div class="flex items-center gap-2 sm:gap-3">
                @if ($otherParticipant)
                    <div class="bg-bg-primary dark:bg-bg-secondary rounded-full p-0.5">
                        ...
                        @else
                            <div
                                class="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-zinc-200 bg-gradient-to-br from-accent to-accent-foreground flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0">
            ...
            <div class="w-md">
                <div class="relative">
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('search conversion') }}"
                    ...
                    @if ($msg->sender_id == auth()->id())
                        ...
                        <div class="flex items-start gap-2 sm:gap-3 flex-row-reverse">
                            ...
                            @else
                                <div
                                    class="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-zinc-200 bg-gradient-to-br from-accent to-accent-foreground flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0">
                                    {{ $msg ? strtoupper(substr($msg->full_name, 0, 2)) : 'S' }}
                                </div>
                            @endif
