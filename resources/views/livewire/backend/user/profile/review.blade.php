    ...
    <div class="flex items-center gap-2 sm:gap-4 mt-5 mb-5">
        <div class="">
            <button x-on:click="$wire.set('type', 'all')"
                class="{{ $type === 'all' ? 'bg-zinc-500 text-text-white' : 'bg-zinc-50 text-zinc-500' }} font-semibold border-1 border-zinc-500 py-1 px-2 xxs:py-2 xxs:px-4 sm:py-3 sm:px-6 rounded-2xl">{{ __('All') }}</button>
        </div>
        <div>
            <button x-on:click="$wire.set('type', 'positive')"
                class="{{ $type === 'positive' ? 'bg-zinc-500 text-text-white' : 'bg-zinc-50 text-zinc-500' }} font-semibold border-1 border-zinc-500 py-1 px-2 xxs:py-2 xxs:px-4 sm:py-3 sm:px-6 rounded-2xl inline-flex items-center gap-2 justify-center">

                {{ __('Positive') }}
                {{-- <x-phosphor name="thumbs-up" variant="solid" class="fill-zinc-500 w-6 h-6 p-0! m-0!" /> --}}
                <x-phosphor name="thumbs-up" variant="solid"
                    class="{{ $type === 'positive' ? 'fill-white' : 'fill-zinc-500' }} w-6 h-6 p-0! m-0!" />
            </button>
        </div>
        <div>
            <button x-on:click="$wire.set('type', 'negative')"
                class="{{ $type === 'negative' ? 'bg-zinc-500 text-text-white' : 'bg-zinc-50 text-zinc-500' }} font-semibold border-1 border-zinc-500 py-1 px-2 xxs:py-2 xxs:px-4 sm:py-3 sm:px-6 rounded-2xl inline-flex items-center gap-2 justify-center">
