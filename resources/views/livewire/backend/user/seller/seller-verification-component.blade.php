    ...
    <div class="max-w-4xl mx-auto">
        ...
        @elseif($currentStep == 1)
            <div class="bg-white rounded-lg shadow-lg p-8">
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-500">Step 1/5</p>
                </div>
                ...
                <div class="max-w-md mx-auto space-y-4 mb-8">
                    ...
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer {{ $accountType === 'company' ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                        <input type="radio" wire:model="accountType" value="company" class="w-5 h-5 text-blue-600">
        ...
        @elseif($currentStep == 3)
            <div class="bg-white rounded-lg shadow-lg p-8">
                ...
                <div class="max-w-md mx-auto space-y-4 mb-8">
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer {{ $sellingExperience === 'new' ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                        <input type="radio" wire:model="sellingExperience" value="new" class="w-5 h-5 text-blue-600">
                        <span class="ml-3 text-lg">New seller (this is my first selling)</span>
                    </label>

                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer {{ $sellingExperience === 'experienced' ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                        <input type="radio" wire:model="sellingExperience" value="experienced" class="w-5 h-5 text-blue-600">
                ...
                @if($accountType === 'individual')
                    ...
                    <div class="max-w-md mx-auto space-y-4 mb-8">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="streetAddress" 
                ...
                @else
                    ...
                    <div class="max-w-md mx-auto space-y-4 mb-8">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="companyCode" 
                                   class="w-full p-3 border rounded-lg"
                                   placeholder="Company code/ID">
                            @error('companyCode') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="companyStreetAddress" 
                                   class="w-full p-3 border rounded-lg"
                                   placeholder="Street address">
                            @error('companyStreetAddress') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="companyCity" 
                                   class="w-full p-3 border rounded-lg"
                                   placeholder="City">
                            @error('companyCity') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                @error('companyCountry') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-semibold mb-2">Postal code</label>
                                <input type="text" wire:model="companyPostalCode" 
                                       class="w-full p-3 border rounded-lg"
                                       placeholder="Postal code">
                                @error('companyPostalCode') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-lg shadow-lg p-8">
                ...
                <div class="max-w-md mx-auto mb-8">
                    ...
                    <ul class="space-y-2 text-sm text-gray-600 mb-6">
                        <li>• Accepted documents: Driver's license, Government issued ID or Passport, international student ID.</li>
                        <li>• Make sure personal details on the document are clearly visible and easy to read.</li>
                    </ul>

                    <div class="mb-4">
                        <input type="file" wire:model="idDocument" accept="image/*" class="hidden" id="idDocument">
                        ...
                        @if($idDocument)
                            <p class="text-green-600 mt-2">✓ File selected: {{ $idDocument->getClientOriginalName() }}</p>
                        @else
                    ...
                    @error('idDocument') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="bg-white rounded-lg shadow-lg p-8">
                    ...
                    <div class="max-w-2xl mx-auto mb-8">
                        <p class="text-gray-600 mb-6 text-center">
                            Please upload documents to prove that the individual who submitted the ID is an owner of your company.
                        </p>

                        <div class="bg-gray-50 rounded-lg p-6 mb-6">
                            <ol class="space-y-3 text-sm text-gray-700">
                                <li>1. Proof of ownership (an extract from a corporate registry or shareholder register) (required)</li>
                                <li>2. Articles of Association (required)</li>
                                <li>3. Proof of registered company address (utility bill or bank statement, not older than 3 months) (required)</li>
                                <li>4. Misc docs (corporate structure, incorporation document, misc. company documents, etc) (optional)</li>
                            </ol>
                        </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700">
                                        <strong>Note:</strong> If your company's owner is another company, you will need to upload documents for both entities and the corporate structure, leading to the UBO
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <input type="file" wire:model="companyDocuments" accept=".jpg,.jpeg,.png,.heic,.pdf,.docx" multiple class="hidden" id="companyDocuments">
                            ...
                            @if(!empty($companyDocuments))
                                <div class="mt-3 space-y-2">
                                    @foreach($companyDocuments as $index => $doc)
                                        <p class="text-green-600 text-sm">✓ File {{ $index + 1 }}: {{ $doc->getClientOriginalName() }}</p>
                                    @endforeach
                                </div>
                            @else
