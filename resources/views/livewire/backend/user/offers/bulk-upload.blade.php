    ...
    <div class="container pb-48">
        ...
        {{-- <div class="bg-bg-secondary rounded-2xl mb-10 p-4 sm:p-10 md:p-20">
            ...
            <div class="mt-10">
                <div class="">
                    <button wire:click=""
                        class="w-full flex justify-between p-4 bg-bg-info hover:bg-zinc-700/30 transition rounded-xl">
                        <div class="flex items-center gap-2 sm:gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-text-white font-normal text-base sm:text-base mt-1">
        ...
        <div class="bg-bg-secondary rounded-2xl mb-10 p-4 sm:p-10 md:p-20">
            ...
            <div class="p-5 sm:p-10 bg-bg-info rounded-2xl">
                <h2 class="text-3xl font-semibold text-text-white mb-2 sm:mb-7">
                    {{ __('Item details') }}
                </h2>

                <div class="w-full">
                    <x-ui.label :for="'name'" :value="'Region:'" class="mb-5! text-2xl! font-semibold!"> </x-ui.label>
                    <x-ui.custom-select :rounded="'rounded'" :label="__('Select a game')" :wireModel="'gameId'" class="border-0! w-full!"
                        :mdWidth="'md:w-full'" :mdLeft="'md:left-0'" :border="'border-transparent'" />
                ...
                <div class="w-full">
                    <x-ui.label :for="'name'" :value="'Server:'" class="mb-5! text-2xl! font-semibold!">
                    </x-ui.label>
                    <x-ui.custom-select :rounded="'rounded'" :label="__('Select Server')" :wireModel="'gameId'"
                ...
                <div class="bg-bg-info p-6 rounded-2xl mt-7">
                    <p class="text-text-white font-normal text-xl"><span class="text-pink-500">{{ __('NOTE!') }}</span> {{ __('Selecting a
                        server also creates offers for both factions (Horde and Alliance). If you don`t need one, delete
                        it afterward.') }}</p>
                </div>
            </div>
            <div class="flex gap-4 justify-center mt-5! sm:mt-10!">
        ...
        <div class="bg-bg-secondary rounded-2xl mb-10 p-4 sm:p-10 md:p-20">
            <h2 class="text-2xl sm:text-40px font-semibold text-center text-text-white mb-3">{{ __('Bulk Upload') }}</h2>

            <div class="bg-bg-info p-6 rounded-2xl my-10">
                <p class="text-pink-500 font-normal text-2xl">{{ __('NOTE!') }}</p>
                <p class="text-text-white font-normal text-xl">
                    {{ __('Bulk upload is available only on currency category at the moment.') }}
                </p>
            </div>

            <div>
                <h3 class="text-text-white font-semibold text-2xl mb-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-pink-500 text-base font-normal">{{ __('WOW Example') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    <x-phosphor name="paperclip" variant="regular" class="w-5 h-5 fill-pink-500" />
                    <p class="text-pink-500 text-base font-normal">{{ __('WOW Mists of Pandaria Example') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    <x-phosphor name="paperclip" variant="regular" class="w-5 h-5 fill-pink-500" />
                    <p class="text-pink-500 text-base font-normal">{{ __('WOW Classic Era Example') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    <x-phosphor name="paperclip" variant="regular" class="w-5 h-5 fill-pink-500" />
                    <p class="text-pink-500 text-base font-normal">{{ __('New World Example') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    <x-phosphor name="paperclip" variant="regular" class="w-5 h-5 fill-pink-500" />
                    <p class="text-pink-500 text-base font-normal">{{ __('Lost Ark Example') }}</p>
                </div>
                <h3 class="text-text-white font-semibold text-2xl mt-6">
            ...
            <div class="flex gap-4 mt-5! sm:mt-10!">
                <div class="flex md:w-auto! ">
                    <x-ui.button class="w-fit! py-2! px-4! text-white">{{ __('Upload images') }}</x-ui.button>
                </div>
                <label for="file-input-alt"
                    class="bg-bg-primary text-text-white font-medium py-2 px-4 rounded-lg cursor-pointer transition-colors inline-block">
                    Choose File
                </label>
                <input id="file-input-alt" type="file" class="hidden">
                
            </div>
            <p class="text-text-white block!">{{ __('Must be CSV and cannot exceed 1MB.') }}</p>
            <div class="flex md:w-auto! mt-9">
                <x-ui.button class="w-fit! py-2! px-4!">{{ __('Upload') }}</x-ui.button>
            </div>
                ...
                <div class="flex gap-4 justify-center mt-5! sm:mt-10!">
                    <div class="flex md:w-auto!">
                        <x-ui.button wire:click="back"
                            class="w-fit! py-2! px-4! text-white">{{ __('Back') }}</x-ui.button>
                    </div>
                    <div class="flex md:w-auto!">
                ...
                <div class="p-5 sm:p-10 bg-bg-info rounded-2xl">
                    ...
                    <div class="w-full">
                        <x-ui.label :for="'region'" :value="'Region:'"
                            class="mb-5! text-2xl! font-semibold!"></x-ui.label>
        ...
        @if ($currentStep === 4)
            <div class="bg-bg-secondary rounded-2xl mb-10 p-4 sm:p-10 md:p-20">
                ...
                <div class="flex gap-4 mt-5! sm:mt-10!">
                    <label for="file-input-alt"
                        class="bg-bg-primary text-text-white font-medium py-2 px-4 rounded-lg cursor-pointer transition-colors inline-block hover:bg-opacity-90">
                        {{ __('Choose File') }}
                    </label>
                    <input id="file-input-alt" type="file" wire:model="file" accept=".csv" class="hidden">
