<div>
    ...
    <div class="container px-24  bg-white dark:bg-zinc-800 mb-20 rounded-3xl">

        <div class="w-full mx-auto p-20">

            {{-- Step 1: Category Selection --}}
            @if ($step === 1)
                <h1 class="text-40px font-semibold text-center text-text-primary mb-3">{{ __('Start selling') }}</h1>
                <h2 class="text-2xl text-center text-text-primary mb-10">{{ __('Choose category') }}</h2>

                <div class="space-y-10">
                    @foreach ($categories as $category)
                        <button wire:click="selectCategory({{ $category->id }}, '{{ $category->name }}')"
                            class="w-full flex items-center justify-between p-4  bg-bg-primary hover:bg-zinc-700/30 transition rounded-xl">
                            <div class="flex items-center space-x-3">
                                <div class="w-16 h-16 ">
                                    <img src="{{ storage_url($category->image) }}" alt="{{ $category->name }}"
                                        class="w-full h-full rounded-xl object-cover">
                                </div>
                                <span class="text-3xl font-semibold text-text-primary">{{ $category->name }}</span>
                            </div>
            ...
            @if ($step === 2)
                ...
                <div class="p-20 bg-bg-primary dark:bg-zinc-400/15 rounded-2xl">
                    ...
                    <div class="w-md flex justify-center mx-auto">
                        @if (count($categoryGames) > 0)
                            <select wire:model="selectedGame"
                                class="w-full bg-zinc-950/50 text-white border-none focus:border-0 focus:ring-0 rounded-lg px-4 py-3">
            ...
            @if ($step === 3)
                ...
                <div class="p-20 bg-zinc-400/15 rounded-2xl">
                    ...
                    <div class="w-md grid grid-cols-1 gap-3 justify-center mx-auto">
                        ...
                        @if (count($platforms) > 0)
                            <select wire:model="selectedPlatform" 
                                class="bg-zinc-700/50  text-white border-none focus:border-0 focus:ring-0 rounded-lg px-4 py-3">
                                <option value="">{{ __('Select Platforms') }}</option>
                    ...
                    <div class="flex justify-center space-x-4 pt-10">
                        ...
                        <button wire:click="updateGameInformation"
            ...
            @if($step === 4)
                ...
                <livewire:backend.user.offers.games.game/>
