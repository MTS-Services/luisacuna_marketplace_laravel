<div x-data="{
    ...
    <div class="flex min-h-screen items-center justify-center p-4">
        <div class="relative w-full max-w-2xl bg-card rounded-2xl shadow-shadow-primary" x-show="showConfigModal"
            ...
            <div class="flex items-center justify-between p-6 border-b border-border">
                <div>
                    <div class=" flex items-center justify-start gap-5">
                        <svg class="w-8 h-8 mx-auto stroke-accent" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div x-show="isLoading" class="mt-1">
                        <div class="h-4 w-48 bg-main rounded animate-pulse"></div>
                    </div>
                </div>
                <button @click="showConfigModal = false; $wire.closeConfigModal()"
                    class="text-text-muted dark:hover:text-gray-200 border border-transparent hover:border-border rounded-full flex items-center justify-center w-10 h-10 hover:bg-gray-100/50 dark:hover:bg-gray-950/50 transition-all duration-300 ease-linear">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-sm text-blue-600 dark:text-blue-400 space-y-1">
                                        ...
                                        <p>{{ __('Category') }}: <span
                                                class="font-medium">{{ $currentCategory?->name }}</span></p>
                                        @if ($gameCategory->configs->count() > 0)
                                            <p>{{ __('Existing Configs') }}: <span
                                                    class="font-medium">{{ $gameCategory->configs->count() }}</span>
                                            </p>
                                        @endif
                                    </div>
                                </div>
