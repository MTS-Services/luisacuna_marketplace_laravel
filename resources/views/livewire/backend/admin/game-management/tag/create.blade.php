<section>
    ...
    <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-text-black dark:text-text-white">{{ __('Tag Create') }}</h2>
    ...
    <div class="glass-card rounded-2xl p-6 mb-6">
        <form wire:submit="save">
            ...
            <div class="mt-6 space-y-4 grid grid-cols-2 gap-5">
                ...
                {{-- Text Color --}}
                <div class="w-full">
                    <x-ui.label value="Text Color" class="mb-1" />
                    <x-ui.input type="color" placeholder="Text Color" id="text_color" wire:model="form.text_color" />
                    <x-ui.input-error :messages="$errors->get('form.text_color')" />
                </div>

                {{-- Sort Order --}}
                <div class="w-full">
                    <x-ui.label value="Sort Order" class="mb-1" />
                    <x-ui.input type="number" placeholder="Sort Order" id="sort_order" wire:model="form.sort_order" />
                    <x-ui.input-error :messages="$errors->get('form.sort_order')" />
                </div>

                {{-- Bg Color --}}
                <div class="w-full">
                    <x-ui.label value="Background Color" class="mb-1" />
                    <x-ui.input type="color" placeholder="Background Color" id="bg_color"
                        wire:model="form.bg_color" />
                    <x-ui.input-error :messages="$errors->get('form.bg_color')" />
                </div>
