                ...
                @if ($step === 1)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div x-data="{ loading: false }" class="relative">
                            <button type="button" @click="loading = true" wire:click="setRecipientTypeAdmin"
                                wire:loading.attr="disabled" :disabled="loading"
                                ...
                                <span x-show="loading"
                                    ...
                                    <svg class="animate-spin h-7 w-7 text-primary" xmlns="http://www.w3.org/2000/svg"
                                ...
                                <span :class="loading ? 'opacity-0' : 'opacity-100'"
                                    class="text-lg font-semibold text-zinc-900 dark:text-white transition-opacity">{{ __('Admin') }}</span>
                                <span :class="loading ? 'opacity-0' : 'opacity-100'"
                                    class="text-sm text-zinc-500 dark:text-zinc-400 mt-1 transition-opacity">{{ __('Send to an administrator') }}</span>
                            </button>
                        </div>

                        {{-- User card --}}
                        <div x-data="{ loading: false }" class="relative">
                            <button type="button" @click="loading = true" wire:click="setRecipientTypeUser"
                                wire:loading.attr="disabled" :disabled="loading"
                                ...
                                <span x-show="loading"
                                    class="absolute inset-0 flex items-center justify-center rounded-2xl bg-white/70 dark:bg-gray-900/70 z-10">
                                    <svg class="animate-spin h-7 w-7 text-accent" xmlns="http://www.w3.org/2000/svg"
                                        fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10"
                                            stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor"
                                ...
                                <span :class="loading ? 'opacity-0' : 'opacity-100'"
                                    class="text-lg font-semibold text-zinc-900 dark:text-white transition-opacity">{{ __('User') }}</span>
                                <span :class="loading ? 'opacity-0' : 'opacity-100'"
                                    class="text-sm text-zinc-500 dark:text-zinc-400 mt-1 transition-opacity">{{ __('Send to a platform user') }}</span>
                            </button>
                        </div>
                    ...
                    <div x-data="{ selectingId: null }" class="space-y-4">
                        ...
                        <div class="relative">
                            <x-ui.input type="text"
                                ...
                                wire:model.live.debounce.300ms="recipientSearch" class="w-full pr-10" />

                            {{-- Animated search icon while typing/loading --}}
                            <div class="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                                <svg wire:loading wire:target="recipientSearch"
                                    class="animate-spin h-4 w-4 text-primary" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10"
                                        stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor"
                                ...
                                <flux:icon wire:loading.remove wire:target="recipientSearch" name="magnifying-glass"
                                    class="w-4 h-4 text-zinc-400 stroke-current" />
                            </div>
                        </div>
                        ...
                        {{-- Results list --}}
                        <div
                            class="border border-zinc-200 dark:border-zinc-700 rounded-xl overflow-hidden max-h-80 overflow-y-auto">

                            {{-- Skeleton rows — shown while searching --}}
                            <div wire:loading wire:target="recipientSearch"
                                class="divide-y divide-zinc-100 dark:divide-zinc-800 w-full">
                                @foreach (range(1, 4) as $skeleton)
                                    <div class="flex items-center gap-3 px-4 py-3 animate-pulse ">
                                        <div class="shrink-0 w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-700">
                                        </div>
                            ...
                            <div wire:loading.remove wire:target="recipientSearch">
                                @forelse ($recipients as $item)
                                    ...
                                    --}}
                                    <div
                                        class="relative border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">

                                        <button type="button" @click="selectingId = {{ $itemId }}"
                                            wire:click="selectRecipient({{ $itemId }}, '{{ $recipientType }}')"
                                            wire:loading.attr="disabled" :disabled="selectingId !== null"
                                            class="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800/80 transition-colors"
                                            :class="selectingId !== null && selectingId !== {{ $itemId }} ?
                                                'opacity-40' : ''">

                                            <span
                                                class="shrink-0 w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center">
                                                <flux:icon name="user" class="w-5 h-5 stroke-current" />
                                            </span>
                                            <div class="min-w-0 flex-1">
                                                <span
                                                    class="block font-medium text-zinc-900 dark:text-white truncate">{{ $name }}</span>
                                                @if ($sub)
                                                    <span
                                                        class="block text-sm text-zinc-500 dark:text-zinc-400 truncate">{{ $sub }}</span>
                                                @endif
                                            </div>
                                            ...
                                            <svg x-show="selectingId === {{ $itemId }}" x-cloak
                                ...
                                @empty
                                    <div class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">
                                        ...
                                        <p>{{ $recipientSearch !== '' ? __('No results found.') : __('Type to search.') }}
                                        </p>
                                    </div>
                                @endforelse
                            </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between gap-4 mb-4">
                            <p class="text-zinc-600 dark:text-zinc-400">
                                {{ __('Sending to') }}: <strong
                                    class="text-zinc-900 dark:text-white">{{ $receiver_name }}</strong>
                            </p>
                            <x-ui.button type="button" variant="tertiary" class="w-auto! py-1!"
                        ...
                        <form wire:submit="save" class="space-y-4">
                            ...
                            <div>
                                ...
                                <x-ui.input type="text" placeholder="{{ __('Title') }}"
                                    wire:model="form.title" />
                                <x-ui.input-error :messages="$errors->get('form.title')" />
                            </div>
                            <div>
                                <x-ui.label value="Message" class="mb-1" />
                                <x-ui.textarea placeholder="{{ __('Message') }}" wire:model="form.message"
                                    rows="4" />
                                <x-ui.input-error :messages="$errors->get('form.message')" />
                            </div>

                            <div class="flex items-center justify-end gap-4 pt-4 border-t">
                                <x-ui.button type="button" wire:click="resetForm" variant="tertiary"
                                    class="w-auto! py-2!">
                                ...
                                <x-ui.button class="w-auto! py-2!" type="submit">
                                    ...
                                    <flux:icon name="arrow-path" wire:loading wire:target="save"
