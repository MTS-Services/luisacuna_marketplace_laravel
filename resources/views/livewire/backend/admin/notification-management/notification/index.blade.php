    ...
    <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                {{-- Mark All Read --}}
                <x-ui.button wire:click="markAllAsRead" wire:loading.attr="disabled" :disabled="$unreadCount === 0"
    ...
    <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="text-sm text-text-muted">
                    <span class="font-medium text-text-primary">{{ $notifications->total() }}</span>
        ...
        <div class="flex items-center gap-4 mt-4 pt-4 border-t border-border">
            <div class="flex items-center gap-2 text-sm">
                ...
                <span class="font-semibold text-text-primary">{{ $stats['total'] }}</span>
            </div>
            <div class="w-px h-4 bg-border"></div>
            <div class="flex items-center gap-2 text-sm">
                <span class="text-text-muted">{{ __('Unread') }}:</span>
                <span class="font-semibold text-zinc-600 dark:text-zinc-400">{{ $stats['unread'] }}</span>
            </div>
            <div class="w-px h-4 bg-border"></div>
            <div class="flex items-center gap-2 text-sm">
                <span class="text-text-muted">{{ __('Read') }}:</span>
                <span class="font-semibold text-secondary-600 dark:text-secondary-400">{{ $stats['read'] }}</span>
            </div>
        </div>
    </div>
        ...
        @else
            ...
            @if ($notifications->count() > 0)
                <div class="hidden sm:flex items-center gap-3 px-6 py-3 bg-hover border-b border-border">
                    ...
                    <label class="text-sm font-medium text-text-secondary cursor-pointer select-none" for="selectAll">
                        {{ __('Select All on Page') }} ({{ $notifications->count() }})
                    </label>
                </div>
            @endif
            ...
            <div class="divide-y divide-border">
                @foreach ($notifications as $notification)
                    @php
                        $id = $notification->id; // raw UUID — stable across renders, used in wire:click / wire:target
                        $encId = encrypt($id); // encrypted — used ONLY for the route URL (href)
                        $isUnread = !$notification->isRead(encrypt(admin()->id), get_class(admin()));
                    @endphp

                    {{--
                        wire:key uses the raw UUID.
                        Encrypted values change every render (new IV), causing
                        Livewire to destroy/recreate DOM nodes instead of diff them.
                    --}}
                    <div wire:key="notification-{{ $id }}"
                        ...
                        <div class="flex items-start gap-4 p-6 {{ $isUnread ? 'pl-8' : 'pl-6' }}">
                            ...
                            <div class="shrink-0 pt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 {{ in_array($id, $selectedNotifications) ? 'opacity-100' : '' }}"
                            ...
                            {{-- Clickable row → single detail page --}}
                            <a href="{{ route('admin.notification.show', $encId) }}" wire:navigate
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    {{-- Meta Information --}}
                                    <div
                                        class="flex items-center flex-wrap gap-x-4 gap-y-2 text-xs text-text-muted mt-3">
                            ...
                            {{-- ─────────────────────────────────────────────────────────
                                 Per-notification action area.
                                 
                                 wire:click and wire:target both use the raw $id (UUID).
                                 Because UUIDs never change, these strings are identical
                                 on every render — so Livewire's JS can always match
                                 the in-flight request to the correct loading targets,
                                 on the 1st click AND on every subsequent click.
                                 ───────────────────────────────────────────────────────── --}}
                            <div class="shrink-0 flex items-center justify-center min-w-[40px]"
                                onclick="event.stopPropagation()">

                                {{-- Spinner: visible only while THIS card's action runs --}}
                                <div wire:loading.flex
                                    wire:target="markAsRead('{{ $id }}'),markAsUnread('{{ $id }}'),deleteNotification('{{ $id }}')"
                                    class="items-center justify-center w-9 h-9" style="display: none;">
                                    <div
                                        class="w-5 h-5 rounded-full border-2 border-border border-t-primary animate-spin">
                                    </div>
                                </div>

                                {{-- Buttons: hidden while THIS card's action runs --}}
                                <div wire:loading.remove
                                    ...
                                    @else
                                        <button wire:click="markAsUnread('{{ $id }}')"
                                            wire:loading.attr="disabled"
