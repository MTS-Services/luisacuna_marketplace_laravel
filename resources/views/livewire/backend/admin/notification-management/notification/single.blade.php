        ...
        <div class="max-w-4xl mx-auto space-y-6 sm:space-y-8">
            @if (!$notification && !$notFound)
                ...
                <div class="glass-card rounded-2xl p-6 sm:p-8 text-center">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            ...
            @elseif($notFound)
                ...
                <div class="glass-card rounded-2xl p-6 sm:p-8 text-center">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-center">
                        <x-ui.button href="{{ route('admin.notification.index') }}" class="w-full sm:w-auto! py-2.5! rounded-lg justify-center">
                            <flux:icon name="arrow-left" class="w-4 h-4 stroke-text-btn-primary group-hover:stroke-text-btn-secondary" />
                            {{ __('View All Notifications') }}
                        </x-ui.button>
                        <x-ui.button href="{{ route('admin.dashboard') }}" class="w-full sm:w-auto! py-2.5! rounded-lg justify-center" variant="tertiary">
                            <flux:icon name="layout-dashboard" class="w-4 h-4 stroke-text-btn-primary group-hover:stroke-text-btn-tertiary" />
                            {{ __('Go to Dashboard') }}
                        </x-ui.button>
                    </div>
                </div>
            @else
                ...
                <div class="glass-card rounded-2xl overflow-hidden">
                    <div class="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
                        ...
                        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 text-center sm:text-left">
                            <div class="relative shrink-0">
                                <div class="relative w-16 h-16 sm:w-20 sm:h-20 glass-card shadow-shadow-primary rounded-full flex items-center justify-center">
                                    <flux:icon name="{{ $data['icon'] ?? 'bell' }}" class="w-6 h-6 sm:w-8 sm:h-8 stroke-primary" />
                                </div>
                            </div>

                            <div class="flex-1 min-w-0 space-y-2 sm:space-y-3 w-full">
                                <h2 id="notification-title" class="text-xl sm:text-2xl md:text-3xl font-bold text-text-primary break-words">
                        ...
                        @if ($data['message'] ?? false)
                            <div>
                                ...
                                <div class="bg-main shadow-shadow-primary rounded-xl p-4 sm:p-6 text-sm sm:text-base text-text-muted leading-relaxed break-words">
                                    {{ $data['message'] }}
                                </div>
                            </div>
                        @endif
                    ...
                    {{-- Actions Footer --}}
                    <div class="bg-gray-50 dark:bg-gray-800/30 px-4 sm:px-6 md:px-8 py-4 sm:py-6 border-t border-gray-200 dark:border-gray-700">
                        <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 sm:gap-6">
                            ...
                            <div class="w-full lg:w-auto flex flex-col sm:flex-row flex-wrap gap-3">
                                @if ($notification->action)
                                    <x-ui.button href="{{ $notification->action }}" target="_blank" :wire='false'
                                        rel="noopener noreferrer" class="w-full sm:w-auto! py-2.5! rounded-lg justify-center">
                                ...
                                <x-ui.button wire:click="markAsUnread" wire:loading.attr="disabled"
                                    class="w-full sm:w-auto! py-2.5! rounded-lg justify-center flex items-center gap-2" variant="secondary"
                                ...
                                <x-ui.button wire:click="deleteNotification" wire:loading.attr="disabled"
                                    ...
                                    class="w-full sm:w-auto! py-2.5! rounded-lg justify-center flex items-center gap-2" variant="tertiary"
                            ...
                            {{-- Right: back to list --}}
                            <div class="w-full lg:w-auto pt-4 lg:pt-0 border-t border-gray-200 dark:border-gray-700 lg:border-t-0">
