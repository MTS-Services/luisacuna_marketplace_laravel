                    ...
                    @forelse ($notifications as $notification)
                        ...
                        <div wire:key="notification-sidebar-{{ $notification->id }}"
                            ...
                            @if ($notification->action)
                                <a href="{{ $notification->action }}" wire:navigate
                                    wire:click="markAsRead('{{ $notification->id }}')"
                                    class="block group hover:bg-hover transition-all duration-200 p-4 {{ $isUnread ? 'pl-5' : 'pl-4' }}">
                                @else
                                    <div wire:click="markAsRead('{{ $notification->id }}')"
                                        class="block group hover:bg-hover transition-all duration-200 p-4 cursor-pointer {{ $isUnread ? 'pl-5' : 'pl-4' }}">
                            @endif
        ...
        <div class="shrink-0 border-t border-border bg-header">
            <div class="flex items-center gap-2 p-4">
                ...
                <x-ui.button href="{{ route('admin.notification.index') }}" wire:navigate
                    class="w-auto! {{ $notifications->count() > 0 ? 'flex-1' : 'w-full' }} py-2! rounded-lg text-sm">
